...
import type { ConversationStateStructure } from "../__generated__/agent/v1/agent_pb.ts";
import {
  AskQuestionRejected,
  AskQuestionResult,
} from "../__generated__/agent/v1/ask_question_tool_pb.ts";
import AgentService from "../api/agent-service.ts";
import {
  LocalResourceProvider,
  type PiToolContext,
} from "../bridge/cursor-to-pi/local-resource-provider/index.ts";
import {
  rejectPendingForSession,
  type ToolExecRequest,
} from "../bridge/cursor-to-pi/tool-bridge.ts";
import { preparePiContext } from "../bridge/pi-context/index.ts";
import {
  buildRunRequest,
  getContextTools,
} from "../bridge/pi-to-cursor/request-builder.ts";
import { CURSOR_API_URL, CURSOR_CLIENT_VERSION } from "../lib/env.ts";
import {
  AgentConnectClient,
  type CheckpointHandler,
  type InteractionListener,
} from "../vendor/agent-client/index.ts";
import type {
  CoreInteractionQuery,
  CoreInteractionResponse,
  CoreInteractionUpdate,
} from "../vendor/agent-core/index.ts";
import {
...
import {
  ...
  type LiveSession,
  setLiveSession,
} from "./agent-stream-hook.ts";
import { toCursorId } from "./model-mapping.ts";
import { type CursorStateStore, createOverlayState } from "./state.ts";

function formatCursorError(error: unknown): string {
  if (error instanceof ConnectError) {
    if (error.code === Code.ResourceExhausted) {
      const detail = error.rawMessage?.trim() || error.message?.trim() || "Cursor quota or plan limit reached";
      return `Cursor resource exhausted: ${detail}`;
    }

    if (error.code === Code.Unavailable) {
      const detail = error.rawMessage?.trim() || error.message?.trim() || "Cursor service unavailable";
      return `Cursor unavailable: ${detail}`;
    }

    return error.rawMessage?.trim() || error.message || String(error);
  }
