...
function renderMarkdown(
  ...
  if (state.turns.length === 0) {
    lines.push("_(No turns recorded.)_", "");
  } else {
  ...
  if (extra.openRisks?.length) lines.push("## Open Risks", "", ...extra.openRisks.map((r) => `- ${r}`), "");
...
export default function grillMeExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      const outputPath = safeOutputPath(ctx.cwd, extras.path);
      await writeFile(outputPath, renderMarkdown(state, extras), "utf8");
      return {
