...
function renderList(scope: TaskScope): string[] {
	...
	return tasks.map(
		(task) => `${task.status === "done" ? "[*]" : task.status === "in_progress" ? "[~]" : "[ ]"} ${task.id} ${task.text}`,
	);
}
...
function buildTaskReminder(options?: { includeProject?: boolean; emphasizeFreshChange?: boolean }): string | undefined {
	...
	return [
		...
		currentSession ? `Current session todo: ${currentSession.text}` : "Current session todo: none",
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		description: "Update a markdown task item: mark done/undone, edit text, move between scopes, reorder, or remove it.",
		parameters: Type.Object({
...
function watchTasksDirectory(): void {
	try {
		...
		// Directory may not exist yet; the snapshot reader tolerates missing files.
	}
}
