...
export function memoryStartupGuidance(): string {
	return [
		"## Prism LTM guidance",
		"",
		"Use memory tools when past context would help:",
		"- memory_recall — durable facts/preferences/decisions (scope=sessions for past session summaries)",
		"- memory_remember — save lasting knowledge (search first; update rather than near-duplicate)",
		"- memory_assess — check coverage before answering from thin air",
		"- Short-lived observations belong in observational-memory; curated topics in mm-wiki.",
		"",
	].join("\n");
}
