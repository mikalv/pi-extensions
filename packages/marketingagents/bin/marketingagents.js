...
if (compareNodeVersions(parsedNodeVersion, parseNodeVersion(MIN_NODE_VERSION)) < 0 || parsedNodeVersion.major > MAX_NODE_MAJOR) {
  ...
  console.error(`marketingagents supports Node.js ${MIN_NODE_VERSION} through ${MAX_NODE_MAJOR}.x (detected ${process.versions.node}).`);
...
const { ensureMarketingAgentsHome } = await import(
  pathToFileURL(resolve(here, "..", "dist", "config", "paths.js")).href
);
ensureMarketingAgentsHome();
