...
const cliCommandNames = ["marketingagents", "ma"];
...
function fail(message) {
	console.error(`[marketingagents] ${message}`);
	process.exit(1);
}

function resolveBundledNodeVersion() {
	const requestedNodeVersion = (
		process.env.MARKETINGAGENTS_BUNDLED_NODE_VERSION ??
		process.env.ADSAGENTS_BUNDLED_NODE_VERSION
	)?.trim();
	if (requestedNodeVersion) {
		if (compareSemver(parseSemver(requestedNodeVersion), parseSemver(minBundledNodeVersion)) < 0) {
			fail(
				`MARKETINGAGENTS_BUNDLED_NODE_VERSION=${requestedNodeVersion} is below the supported floor ${minBundledNodeVersion}`,
			);
		}
...
function writeLauncher(bundleRoot, target) {
	if (target.launcher === "unix") {
		for (const commandName of cliCommandNames) {
			const launcherPath = resolve(bundleRoot, commandName);
			writeFileSync(
				launcherPath,
				[
					"#!/bin/sh",
					...
					'exec "$ROOT/node/bin/node" "$ROOT/app/bin/marketingagents.js" "$@"',
					"",
				].join("\n"),
				"utf8",
			);
	...
	for (const commandName of cliCommandNames) {
		writeFileSync(
			resolve(bundleRoot, `${commandName}.cmd`),
			[
				"@echo off",
				"setlocal",
				'set "ROOT=%~dp0"',
				'if "%ROOT:~-1%"=="\\" set "ROOT=%ROOT:~0,-1%"',
				'"%ROOT%\\node\\node.exe" "%ROOT%\\app\\bin\\marketingagents.js" %*',
				"",
			].join("\r\n"),
			"utf8",
		);
		writeFileSync(
			resolve(bundleRoot, `${commandName}.ps1`),
			[
				'$Root = Split-Path -Parent $MyInvocation.MyCommand.Path',
				'& "$Root\\node\\node.exe" "$Root\\app\\bin\\marketingagents.js" @args',
				"",
			].join("\r\n"),
			"utf8",
		);
...
function validateBundle(bundleRoot, target) {
	...
	run(nodeExecutable, ["-e", "require('./app/.marketingagents/npm/node_modules/better-sqlite3'); console.log('better-sqlite3 ok')"], {
		cwd: bundleRoot,
	});
}
...
function main() {
	...
	const stagingRoot = mkdtempSync(join(tmpdir(), "marketingagents-native-"));
	const outDir = resolve(appRoot, "dist", "release");
	const bundleRoot = resolve(stagingRoot, `marketingagents-${packageJson.version}-${target.id}`);
	...
	const appMarketingAgentsDir = resolve(appDir, ".marketingagents");
	extractTarball(resolve(appMarketingAgentsDir, "runtime-workspace.tgz"), appMarketingAgentsDir, "-xzf");
	rmSync(resolve(appMarketingAgentsDir, "runtime-workspace.tgz"), { force: true });
	...
	console.log(`[marketingagents] native bundle ready: ${archivePath}`);
}
