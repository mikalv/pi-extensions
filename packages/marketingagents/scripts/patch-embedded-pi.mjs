...
import { MARKETINGAGENTS_LOGO_HTML } from "../logo.mjs";
...
const marketingagentsHome = resolve(
	process.env.MARKETINGAGENTS_HOME ?? process.env.ADSAGENTS_HOME ?? homedir(),
	".marketingagents",
);
const marketingagentsNpmPrefix = resolve(marketingagentsHome, "npm-global");
process.env.MARKETINGAGENTS_NPM_PREFIX = marketingagentsNpmPrefix;
process.env.NPM_CONFIG_PREFIX = marketingagentsNpmPrefix;
process.env.npm_config_prefix = marketingagentsNpmPrefix;
...
if (!piPackageRoot) {
	console.warn("[marketingagents] pi-coding-agent not found, skipping Pi patches");
}
...
const workspaceRoot = resolve(appRoot, ".marketingagents", "npm", "node_modules");
...
const settingsPath = resolve(appRoot, ".marketingagents", "settings.json");
const workspaceDir = resolve(appRoot, ".marketingagents", "npm");
const workspacePackageJsonPath = resolve(workspaceDir, "package.json");
const workspaceManifestPath = resolve(workspaceDir, ".runtime-manifest.json");
const workspaceArchivePath = resolve(appRoot, ".marketingagents", "runtime-workspace.tgz");
const workspaceSetupLockDir = resolve(appRoot, ".marketingagents", ".workspace-setup.lock");
const globalNodeModulesRoot = resolve(marketingagentsNpmPrefix, "lib", "node_modules");
...
function resolvePackageManager() {
	...
	const requested = (process.env.MARKETINGAGENTS_PACKAGE_MANAGER ?? process.env.ADSAGENTS_PACKAGE_MANAGER)?.trim();
...
function installWorkspacePackages(packageSpecs) {
	...
	if (!packageManager) {
		process.stderr.write(
			"[marketingagents] no supported package manager found; install npm, pnpm, or bun, or set MARKETINGAGENTS_PACKAGE_MANAGER.\n",
		);
		return false;
	}
	...
	if (result.status !== 0) {
		process.stderr.write(`[marketingagents] ${packageManager} failed while setting up bundled packages.\n`);
		return false;
	}
...
function restorePackagedWorkspace(packageSpecs) {
	...
	mkdirSync(resolve(appRoot, ".marketingagents"), { recursive: true });

	const result = spawnSync("tar", ["-xzf", workspaceArchivePath, "-C", resolve(appRoot, ".marketingagents")], {
	...
	while (true) {
		try {
			...
			if (Date.now() - startedAt > WORKSPACE_SETUP_LOCK_STALE_MS) {
				throw new Error("Timed out waiting for another MarketingAgents process to finish package setup.");
			}
			sleepSync(100);
		}
	}
...
function ensurePackageWorkspaceUnlocked() {
	...
	writeFileSync(
		...
		JSON.stringify({ name: "marketingagents-packages", private: true }, null, 2) + "\n",
		"utf8",
	);
	...
	const spinner = setInterval(() => {
		...
		process.stderr.write(`\r${frames[frame++ % frames.length]} setting up marketingagents... ${elapsed}s`);
	}, 80);
...
function ensurePandoc() {
	...
	if ((process.env.MARKETINGAGENTS_SKIP_PANDOC_INSTALL ?? process.env.ADSAGENTS_SKIP_PANDOC_INSTALL) === "1") return;
	...
	console.log("[marketingagents] installing pandoc...");
	const result = spawnSync(brewPath, ["install", "pandoc"], {
		stdio: "inherit",
		timeout: 300000,
	});
	if (result.status !== 0) {
		console.warn("[marketingagents] warning: pandoc install failed, run `marketingagents --setup-preview` later");
	}
}
...
if (packageJsonPath && existsSync(packageJsonPath)) {
	...
	if (pkg.piConfig?.name !== "marketingagents" || pkg.piConfig?.configDir !== ".marketingagents") {
		pkg.piConfig = {
			...(pkg.piConfig || {}),
			name: "marketingagents",
			configDir: ".marketingagents",
		};
...
for (const entryPath of [cliPath, bunCliPath].filter(Boolean)) {
	...
	if (cliSource.includes('process.title = "pi";')) {
		cliSource = cliSource.replace('process.title = "pi";', 'process.title = "marketingagents";');
	}
	const stdinErrorGuard = [
		"const marketingagentsHandleStdinError = (error) => {",
		...
		"    }",
		"};",
		'process.stdin?.on?.("error", marketingagentsHandleStdinError);',
	].join("\n");
	if (!cliSource.includes('process.stdin?.on?.("error", marketingagentsHandleStdinError);')) {
	...
	if (interactiveModeSource.includes("`π - ${sessionName} - ${cwdBasename}`")) {
		writeFileSync(
			...
			interactiveModeSource
				.replace("`π - ${sessionName} - ${cwdBasename}`", "`marketingagents - ${sessionName} - ${cwdBasename}`")
				.replace("`π - ${cwdBasename}`", "`marketingagents - ${cwdBasename}`"),
			"utf8",
		);
	}
}
...
if (existsSync(sessionSearchIndexerPath)) {
	const source = readFileSync(sessionSearchIndexerPath, "utf8")
		.replaceAll("ADSAGENTS_SESSION_DIR", "MARKETINGAGENTS_SESSION_DIR");
	const original = 'const sessionsDir = path.join(os.homedir(), ".pi", "agent", "sessions");';
	const replacement =
		'const sessionsDir = process.env.MARKETINGAGENTS_SESSION_DIR ?? process.env.PI_SESSION_DIR ?? path.join(os.homedir(), ".pi", "agent", "sessions");';
...
if (oauthPagePath && existsSync(oauthPagePath)) {
	...
	let changed = false;
	const target = `const LOGO_SVG = \`${MARKETINGAGENTS_LOGO_HTML}\`;`;
...
if (existsSync(piMemoryPath)) {
	let source = readFileSync(piMemoryPath, "utf8")
		.replaceAll("ADSAGENTS_MEMORY_DIR", "MARKETINGAGENTS_MEMORY_DIR")
		.replaceAll("ADSAGENTS_NODE_EXECUTABLE", "MARKETINGAGENTS_NODE_EXECUTABLE")
		.replaceAll("ADSAGENTS_EXECUTABLE", "MARKETINGAGENTS_EXECUTABLE")
		.replaceAll("ADSAGENTS_BIN_PATH", "MARKETINGAGENTS_BIN_PATH");
	const memoryOriginal = 'const MEMORY_DIR = join(homedir(), ".pi", "memory");';
	const memoryReplacement =
		'const MEMORY_DIR = process.env.MARKETINGAGENTS_MEMORY_DIR ?? process.env.PI_MEMORY_DIR ?? join(homedir(), ".pi", "memory");';
	...
	const execReplacement = [
		'const execBinary = process.env.MARKETINGAGENTS_NODE_EXECUTABLE || process.env.MARKETINGAGENTS_EXECUTABLE || "pi";',
		'      const execArgs = process.env.MARKETINGAGENTS_BIN_PATH',
		'        ? [process.env.MARKETINGAGENTS_BIN_PATH, "--prompt", prompt]',
		'        : ["-p", prompt, "--print"];',
