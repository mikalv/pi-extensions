...
function Resolve-VersionMetadata {
  ...
  return [PSCustomObject]@{
    ...
    DownloadUrl = if ($env:MARKETINGAGENTS_INSTALL_SKILLS_ARCHIVE_URL) {
      $env:MARKETINGAGENTS_INSTALL_SKILLS_ARCHIVE_URL
    } elseif ($env:ADSAGENTS_INSTALL_SKILLS_ARCHIVE_URL) {
      $env:ADSAGENTS_INSTALL_SKILLS_ARCHIVE_URL
    } else {
      "https://github.com/youmake-ai/marketingagents/archive/refs/tags/v$resolvedVersion.zip"
    }
  }
}
...
function Resolve-InstallDir {
  ...
  if ($ResolvedScope -eq "Repo") {
    return Join-Path (Get-Location) ".agents\skills\marketingagents"
  }

  $codexHome = if ($env:CODEX_HOME) { $env:CODEX_HOME } else { Join-Path $HOME ".codex" }
  return Join-Path $codexHome "skills\marketingagents"
}

$resolvedTargetDir = if ($TargetDir) {
  $TargetDir
} elseif ($env:MARKETINGAGENTS_SKILLS_DIR) {
  $env:MARKETINGAGENTS_SKILLS_DIR
} elseif ($env:ADSAGENTS_SKILLS_DIR) {
  $env:ADSAGENTS_SKILLS_DIR
} else {
  ""
}
...
$installDir = Resolve-InstallDir -ResolvedScope $Scope -ResolvedTargetDir $resolvedTargetDir

$tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) ("marketingagents-skills-install-" + [System.Guid]::NewGuid().ToString("N"))
New-Item -ItemType Directory -Path $tmpDir | Out-Null

try {
  $archivePath = Join-Path $tmpDir "marketingagents-skills.zip"
  $extractDir = Join-Path $tmpDir "extract"

  Write-Host "==> Downloading MarketingAgents skills $resolvedVersion"
  ...
  if (-not $sourceRoot) {
    throw "Could not find extracted MarketingAgents archive."
  }
  ...
  Write-Host "MarketingAgents skills $resolvedVersion installed successfully."
} finally {
  if (Test-Path $tmpDir) {
