...
VERSION="latest"
SCOPE="${MARKETINGAGENTS_SKILLS_SCOPE:-${ADSAGENTS_SKILLS_SCOPE:-user}}"
TARGET_DIR="${MARKETINGAGENTS_SKILLS_DIR:-${ADSAGENTS_SKILLS_DIR:-}}"
...
download_file() {
  ...
  echo "curl or wget is required to install MarketingAgents skills." >&2
  exit 1
}
...
resolve_target_dir() {
  ...
  case "$SCOPE" in
    repo)
      printf '%s/.agents/skills/marketingagents\n' "$PWD"
      ;;
    user)
      codex_home="${CODEX_HOME:-$HOME/.codex}"
      printf '%s/skills/marketingagents\n' "$codex_home"
      ;;
    *)
...
archive_url="${MARKETINGAGENTS_INSTALL_SKILLS_ARCHIVE_URL:-${ADSAGENTS_INSTALL_SKILLS_ARCHIVE_URL:-}}"
if [ -z "$archive_url" ]; then
  case "$git_ref" in
    main)
      archive_url="https://github.com/youmake-ai/marketingagents/archive/refs/heads/main.tar.gz"
      ;;
    v*)
      archive_url="https://github.com/youmake-ai/marketingagents/archive/refs/tags/${git_ref}.tar.gz"
      ;;
  esac
fi
...
step "Installing MarketingAgents skills ${resolved_version} (${SCOPE})"
...
archive_path="$tmp_dir/marketingagents-skills.tar.gz"
...
printf 'MarketingAgents skills %s installed successfully.\n' "$resolved_version"
