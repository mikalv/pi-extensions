...
const RESOLVE_PI_AGENT_DIR_CONFIG_LINE =
	'	const configured = process.env.MARKETINGAGENTS_CODING_AGENT_DIR?.trim() || process.env.ADSAGENTS_CODING_AGENT_DIR?.trim() || process.env.PI_CODING_AGENT_DIR?.trim();';

const RESOLVE_PI_AGENT_DIR_HELPER = [
	"function resolvePiAgentDir(): string {",
	RESOLVE_PI_AGENT_DIR_CONFIG_LINE,
...
export function patchPiSubagentsSource(relativePath, source) {
	let patched = source
		.replaceAll("ADSAGENTS_CODING_AGENT_DIR", "MARKETINGAGENTS_CODING_AGENT_DIR")
		.replaceAll("ADSAGENTS_PI_CLI_PATH", "MARKETINGAGENTS_PI_CLI_PATH")
		.replaceAll("adsagentsPiCliPath", "marketingagentsPiCliPath");
	...
	switch (relativePath) {
		...
		case "pi-spawn.ts":
			patched = replaceAll(
				...
				[
					"\tconst argv1 = deps.argv1 ?? process.argv[1];",
					"\tconst marketingagentsPiCliPath = process.env.MARKETINGAGENTS_PI_CLI_PATH;",
					"\tif (marketingagentsPiCliPath) {",
					"\t\tconst cliPath = normalizePath(marketingagentsPiCliPath);",
