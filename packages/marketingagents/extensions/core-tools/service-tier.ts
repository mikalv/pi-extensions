...
const MARKETINGAGENTS_SERVICE_TIERS = [
	"auto",
	"default",
	"flex",
	"priority",
...
type MarketingAgentsServiceTier = (typeof MARKETINGAGENTS_SERVICE_TIERS)[number];

const SERVICE_TIER_SET = new Set<string>(MARKETINGAGENTS_SERVICE_TIERS);
const OPENAI_SERVICE_TIERS = new Set<MarketingAgentsServiceTier>(["auto", "default", "flex", "priority"]);
const ANTHROPIC_SERVICE_TIERS = new Set<MarketingAgentsServiceTier>(["auto", "standard_only"]);
...
function resolveMarketingAgentsSettingsPath(): string {
	const configured = (
		process.env.PI_CODING_AGENT_DIR ??
		process.env.MARKETINGAGENTS_CODING_AGENT_DIR ??
		process.env.ADSAGENTS_CODING_AGENT_DIR
	)?.trim();
	const agentDir = configured
		? configured.startsWith("~/")
			? resolve(homedir(), configured.slice(2))
			: resolve(configured)
		: resolve(homedir(), ".marketingagents", "agent");
	return resolve(agentDir, "settings.json");
}

function normalizeServiceTier(value: string | undefined): MarketingAgentsServiceTier | undefined {
	if (!value) return undefined;
	const normalized = value.trim().toLowerCase();
	return SERVICE_TIER_SET.has(normalized) ? (normalized as MarketingAgentsServiceTier) : undefined;
}

function getConfiguredServiceTier(settingsPath: string): MarketingAgentsServiceTier | undefined {
	try {
...
function setConfiguredServiceTier(settingsPath: string, tier: MarketingAgentsServiceTier | undefined): void {
...
function resolveActiveServiceTier(settingsPath: string): MarketingAgentsServiceTier | undefined {
	return normalizeServiceTier(process.env.MARKETINGAGENTS_SERVICE_TIER ?? process.env.ADSAGENTS_SERVICE_TIER) ?? getConfiguredServiceTier(settingsPath);
}

function resolveProviderServiceTier(
	provider: string | undefined,
	tier: MarketingAgentsServiceTier | undefined,
): MarketingAgentsServiceTier | undefined {
...
function parseRequestedTier(rawArgs: string): MarketingAgentsServiceTier | null | undefined {
...
export function registerServiceTierControls(pi: ExtensionAPI): void {
	pi.on("before_provider_request", (event, ctx) => {
		...
		const activeTier = resolveActiveServiceTier(resolveMarketingAgentsSettingsPath());
	...
	pi.registerCommand("service-tier", {
		...
		handler: async (args, ctx) => {
			const settingsPath = resolveMarketingAgentsSettingsPath();
			...
			if (requested === undefined && !args.trim()) {
				...
				const selected = await selectOption(
					...
					[
						...
						...MARKETINGAGENTS_SERVICE_TIERS.map((tier) => ({
