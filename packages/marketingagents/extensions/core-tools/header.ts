...
import {
	APP_ROOT,
	MARKETINGAGENTS_AGENT_LOGO,
	MARKETINGAGENTS_VERSION,
} from "./shared.js";
...
async function buildAgentCatalogSummary(): Promise<{ agents: string[]; chains: string[] }> {
	...
	const chains: string[] = [];
	try {
		const entries = await readdir(resolvePath(APP_ROOT, ".marketingagents", "agents"), { withFileTypes: true });
...
function getMarketingWorkflows(pi: ExtensionAPI): WorkflowInfo[] {
...
export function installMarketingAgentsHeader(
	pi: ExtensionAPI,
	...
	return cache.agentSummaryPromise.then((agentData) => {
		...
		const workflows = getMarketingWorkflows(pi);
		...
		ctx.ui.setHeader((_tui, theme) => ({
			render(width: number): string[] {
				...
				push("");
				if (cardW >= 70) {
					const maxLogoW = Math.max(...MARKETINGAGENTS_AGENT_LOGO.map((l) => l.length));
					const logoOffset = " ".repeat(Math.max(0, Math.floor((cardW - maxLogoW) / 2)));
					for (const logoLine of MARKETINGAGENTS_AGENT_LOGO) {
						push(theme.fg("accent", theme.bold(`${logoOffset}${truncateVisible(logoLine, cardW)}`)));
					}
					push("");
				}

				const versionTag = ` v${MARKETINGAGENTS_VERSION} `;
				...
				if (useWideLayout) {
					...
					const rightLines: string[] = [
						"",
						theme.fg("accent", theme.bold("Marketing Workflows")),
					];
					...
					push(sep());
					push(row(theme.fg("accent", theme.bold("Marketing Workflows"))));
