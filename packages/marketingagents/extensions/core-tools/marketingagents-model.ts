...
function resolveMarketingAgentsAgentDir(): string {
	const configured = process.env.PI_CODING_AGENT_DIR ?? process.env.MARKETINGAGENTS_CODING_AGENT_DIR ?? process.env.ADSAGENTS_CODING_AGENT_DIR;
	if (configured?.trim()) {
		return resolve(expandHomePath(configured.trim()));
	}
	return resolve(homedir(), ".marketingagents", "agent");
}
...
export function registerMarketingAgentsModelCommand(pi: ExtensionAPI): void {
	pi.registerCommand("marketingagents-model", {
		description: "Open MarketingAgents model menu (main + per-subagent overrides).",
		handler: async (_args, ctx) => {
			if (!ctx.hasUI) {
				ctx.ui.notify("marketingagents-model requires interactive mode.", "error");
				return;
			}
			...
			try {
				...
				const agentDir = resolveMarketingAgentsAgentDir();
