...
export function registerInitCommand(pi: ExtensionAPI): void {
	pi.registerCommand("init", {
		description: getExtensionCommandSpec("init")?.description ?? "Initialize AGENTS.md and session-log folders for a marketing workspace.",
...
export function registerOutputsCommand(pi: ExtensionAPI): void {
	pi.registerCommand("outputs", {
		description: "Browse marketing artifacts (context, campaigns, creatives, reports, and notes).",
		handler: async (_args, ctx) => {
			const items = await collectArtifacts(ctx.cwd);
			if (items.length === 0) {
				ctx.ui.notify("No artifacts found. Use /start, /context, /research, or /strategy to create one.", "info");
				return;
			}
