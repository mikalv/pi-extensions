...
This page documents configuration for `@meeh/mm-observational-memory` (ported from `pi-observational-memory` V3).

Settings keys accepted: `mm-observational-memory` (preferred) and legacy `observational-memory`. Old V2 keys are not aliases; they are ignored. If you are upgrading, read [Migrating from V2](#migrating-from-v2).
