...
import { promoteEnabledFromEnv, promoteReflections } from "../promote.js";
...
async function runReflectorStage(
	...
	// Promote durable reflections into Prism LTM + wiki (best-effort; never blocks OM).
	const promoteFlags = promoteEnabledFromEnv();
	if (promoteFlags.toPrism || promoteFlags.toWiki) {
		try {
			const promoted = await promoteReflections({
				cwd: ctx.cwd,
				reflections,
				...promoteFlags,
			});
			debugLog("reflector.promoted", promoted);
			if (ctx.hasUI && (promoted.prismIndexed > 0 || promoted.wikiAppended > 0)) {
				ctx.ui?.notify(
					`Observational memory: promoted ${promoted.prismIndexed}→Prism, ${promoted.wikiAppended}→wiki`,
					"info",
				);
			}
			if (promoted.errors.length && ctx.hasUI) {
				ctx.ui?.notify(`Observational memory promote: ${promoted.errors[0]}`, "warning");
			}
		} catch (error) {
			debugLog("reflector.promote_failed", {
